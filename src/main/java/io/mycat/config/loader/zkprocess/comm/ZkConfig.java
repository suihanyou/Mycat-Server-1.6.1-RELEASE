...
import io.mycat.config.util.IpTools;
...
/**
 * 进行zk的配制信息 源文件名：ZkConfig.java 文件版本：1.0.0 创建作者：liujun 创建日期：2016年9月15日
 * 修改作者：liujun 修改日期：2016年9月15日 文件描述：TODO 版权所有：Copyright 2016 zjhz, Inc. All
 * Rights Reserved.
 */
public class ZkConfig {
    ...
    private static Properties LoadMyidPropersites() {
        ...
        try {
            ipString = IpTools.getMyIPLocal(pros.getProperty(ZkParamCfg.ZK_NETWORK_NAME.getKey()));
            if (ipString == null || ipString.equals("")) {
                throw new RuntimeException("failed to get ip string");
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("failed to get ip string");
        ...
        pros.setProperty(ZkParamCfg.ZK_CFG_MYID.getKey(), pros.getProperty(ZkParamCfg.ZK_CFG_MYID.getKey()) + "~"
                + ipString);
        pros.setProperty(ZkParamCfg.NODE_IP_STRING.getKey(), ipString);
        return pros;
