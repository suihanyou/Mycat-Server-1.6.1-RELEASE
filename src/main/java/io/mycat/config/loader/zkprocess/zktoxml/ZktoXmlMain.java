...
public class ZktoXmlMain {
    ...
    public static void loadZktoFile() throws Exception {
        ...
        String id0 = ZkConfig.getInstance().getValue(ZkParamCfg.ACL_ID0).equals("") ? "wang" : ZkConfig.getInstance()
                .getValue(ZkParamCfg.ACL_ID0);
        String id1 = ZkConfig.getInstance().getValue(ZkParamCfg.ACL_ID1).equals("") ? "yang" : ZkConfig.getInstance()
                .getValue(ZkParamCfg.ACL_ID1);
        zkConn = buildConnection(ZkConfig.getInstance().getValue(ZkParamCfg.ZK_CFG_URL), id0, id1);
        if (Boolean.parseBoolean(ZkConfig.getInstance().getValue(ZkParamCfg.ZK_CFG_FLAG))) {
            // 获得公共的xml转换器对象
     ...
     * @author shy
     * */
    public static void registerMycat(int port, Map<String, UserConfig> userConfigs) throws Exception {
        ...
        zkConn.create()
                ...
                .forPath(
                        subPath,
                        FastJSONUtils.toJSONByteArray(new MycatNodeVO(nodeInfo, ZkConfig.getInstance().getValue(
    ...
    private static CuratorFramework buildConnection(String url, final String id0, final String id1) {
        ...
        try {
            ...
            if (curatorFramework.getZookeeperClient().isConnected()) {
                return curatorFramework.usingNamespace(ZookeeperPath.FLOW_ZK_PATH_BASE.getKey());
            }
