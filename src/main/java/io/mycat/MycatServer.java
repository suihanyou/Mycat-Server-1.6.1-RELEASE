...
public class MycatServer {
    ...
    private MycatServer() {
        ...
        // 到zk上注册端口服务
        // 如果需要在zk上注册
        if (Boolean.parseBoolean(ZkConfig.getInstance().getValue(ZkParamCfg.ZK_REG_FLAG))) {
            try {
                ZktoXmlMain.registerMycat(this.config.getSystem().getServerPort(), this.config.getUsers());
            } catch (Exception e) {
                e.printStackTrace();
                throw new RuntimeException("注册mycat服务失败");
            }
        }
        ...
        // catlet加载器
        catletClassLoader = new DynaClassLoader(SystemConfig.getHomePath() + File.separator + "catlet", config
                .getSystem().getCatletClassCheckSeconds());
    ...
    public void startup() throws IOException {
        ...
        if (aio) {
            ...
            NIOReactorPool reactorPool = new NIOReactorPool(DirectByteBufferPool.LOCAL_BUF_THREAD_PREX + "NIOREACTOR",
                    processors.length);
